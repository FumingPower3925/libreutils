import { ImageCompressor, type CompressionOptions, type CompressionResult } from './tool';
...
export function renderImageCompressorPage(): HTMLElement {
    ...
    <style>
      ...
      .select { width: 100%; padding: 0.625rem 0.75rem; padding-right: 3rem; border: 1px solid var(--lu-border, #e5e7eb); border-radius: 0.5rem; background: var(--lu-bg-card, white); color: var(--lu-text-primary, #111827); cursor: pointer; appearance: none; -webkit-appearance: none; -moz-appearance: none; font-size: 0.875rem; box-sizing: border-box; }
    ...
    <header class="header">
      ...
      <p class="subtitle">Compress and resize images with side-by-side preview</p>
    ...
    <div class="card">
      <div class="drop-zone" id="drop-zone">
        ...
        <div style="font-size: 0.8rem; color: var(--lu-text-muted, #9ca3af); margin-top: 0.5rem;">Supports JPEG, PNG, WebP</div>
        <input type="file" id="file-input" accept=".jpg,.jpeg,.png,.webp" hidden>
      </div>
    </div>
...
function setupEventListeners(container: HTMLElement) {
    ...
    // Cleanup handler for revoking object URLs
    cleanupHook = () => {
    ...
    compressBtn.addEventListener('click', async () => {
        ...
        try {
            const options: CompressionOptions = {
                ...
                outputFormat: formatSelect.value as CompressionOptions['outputFormat'],
            };
            ...
            originalDimensions.textContent = `${result.originalWidth} x ${result.originalHeight}`;
